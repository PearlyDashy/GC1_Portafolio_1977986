...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Actividad de Portafolio No.2: Cubo Rubik", NULL, NULL);
    if (window == NULL)
    {
    ...
    unsigned char* data = stbi_load("C:\\gl\\resources\\textures\\RubikColorz.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
   ...
   ourShader.setInt("texture2", 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // render
        // ------
        glClearColor(0.51f, 0.56f, 0.66f, 0.5f); //Cambio de color en el fondo, Rojo / Verde / Azul / Alfa 
        ...
        model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(increase));
